...
struct EventListener: public Viewer::Listener {
  ...
  void handle_node_selection_event(int i) {
    if(i != -1) {
      std::cout << "handle_node_selection_event! " << i << std::endl;
      g.node(i).position().z += 0.2;
      ...
      std::cout << "handle_node_selection_event! null" << std::endl;
    }
