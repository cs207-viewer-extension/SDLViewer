...
class SDLViewer {
 ...
 public:
  ...
  /** Listener Struct with virtual function */
  struct Listener{ 
  	virtual void handle(SDL_Event e) = 0;
    virtual void handle_keyboard_event(char k) = 0;
    virtual void handle_mouse_left_click_event() = 0;
    virtual void handle_mouse_right_click_event() = 0;
    virtual void handle_node_selection_event(int i) = 0;  //node index
    virtual void handle_mouse_move_event() = 0;
  };
  
  
	/** Function to add listener for a given user event */
  void add_listener(std::unique_ptr<Listener> l) {
    if (l == nullptr)
      std::cerr << "Can not register null pointer." << std::endl;
...
//   Point function get3dPoint(Point point2D, int width,
...
// 	}
  ...
  void handle_event(SDL_Event event) {
    ...
    // Loop through all the listeners and call their handle function with the current event
    // TODO: Do some calculation when apporopiate. e.g: find closest node
    for (auto const& el : listeners_){

      switch (event.type) {

        // The mouse moved over the screen
        case SDL_MOUSEMOTION: {
          el->handle_mouse_move_event();
        } break;
        ...
        case SDL_MOUSEBUTTONDOWN: {  
          ...
          // Left mouse button is down
          if (event.motion.state == SDL_BUTTON(1)){
            el->handle_mouse_left_click_event();
            //Get mouse position
            int x = 0, y = 0;
            SDL_GetMouseState( &x, &y );
                 
            //std::cerr <<"Mouse position is : " << event.motion.x <<  ' '  << event.motion.y <<std::endl;
            //std::cerr <<"Mouse position is : " << x <<  ' '  << y <<std::endl;
            
            el->handle_node_selection_event(render_picking(x,y));
          }
          
          // Right mouse button is down
          if (event.motion.state != SDL_BUTTON(1)) 
            el->handle_mouse_right_click_event();

        } break;


        case SDL_KEYDOWN: {
          // Pass the pressed key to the callback
          el->handle_keyboard_event(event.key.keysym.sym);
        } break;
      }
   ...
   */
  void render() {
  	//std::cerr << "Render requested!"<< std::endl;
    safe_lock mutex(this);
 ...
 /** Render to back buffer for color picking
   *
   * Double buffered rendering to the back buffer using OpenGL
   * Returns index of node clicked on by user, if none, returns -1
   */
  int render_picking(int x, int y) {
    safe_lock mutex(this);
    
  	//Disable texture, fog and lighting before rendering color version of objects
    ...
    glGetIntegerv(GL_VIEWPORT, viewport);
		...
		//Read in pixel around mouse from the current buffer
